...
export async function ordersCsv(_req, res) {
...
export async function revenueSummary(_req, res) {
    const now = new Date();
...
export async function topPackages(req, res) {
...
export async function upcomingEvents(req, res) {
...
export async function pendingAging(_req, res) {
    ...
    const oldestPendingDays = oldest
        ? Math.max(0, Math.ceil((Date.now() - oldest.createdAt.getTime()) / (1000 * 60 * 60 * 24)))
        : 0;
    return res.json({
        status: "success",
...
export async function ordersTrend(req, res) {
    const days = Number(req.query.days ?? 30);
    const start = new Date();
...
export async function statusDistribution(_req, res) {
